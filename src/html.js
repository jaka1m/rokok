...
export class Document {
    ...
    constructor(request, env, cachedPrxList, isApiReady) {
        this.html = baseHTML;
        this.request = request;
        this.url = new URL(this.request.url);
        this.env = env;
        this.cachedPrxList = cachedPrxList;
    ...
    buildCountryFlag() {
        ...
        const flagList = [];
        for (const proxy of this.cachedPrxList) {
    ...
    build() {
        ...
        this.html = this.html.replaceAll("PLACEHOLDER_API_READY", this.isApiReady ? "block" : "hidden");
        ...
        this.html = this.html.replaceAll('PLACEHOLDER_CHECK_PROXY_URL', `https://${SERVICE_NAME(this.env)}.${ROOT_DOMAIN(this.env)}/check?target=`);
        this.html = this.html.replaceAll('PLACEHOLDER_ROOT_DOMAIN', `${SERVICE_NAME(this.env)}.${ROOT_DOMAIN(this.env)}`);
    ...
    buildDropdowns() {
        ...
        // Country Dropdown
        const countries = new Set(this.cachedPrxList.map(p => p.country));
        ...
        // Host Dropdown
        const hosts = [ROOT_DOMAIN(this.env), 'ava.game.naver.com', 'investor.fb.com'];
        let hostOptions = '';
